...
var AutoCompounderSTRK = class {
  ...
  async init() {
    ...
    this.contract = new import_starknet3.Contract(cls.abi, this.addr.address, this.config.provider);
  ...
  async balanceOfUnderlying(user) {
    ...
    const assets = await this.contract.convert_to_assets(import_starknet3.uint256.bnToUint256(balanceShares.toWei()));
...
var Store = class {
  ...
  getAccount() {
    let data = JSON.parse((0, import_fs.readFileSync)(`${this.storeConfig.SECRET_FILE_FOLDER}/account_${process.env.NETWORK}.json`, {
      encoding: "utf-8"
    }));
    return new import_starknet4.Account(this.config.provider, data.address, data.pk);
  }
};
