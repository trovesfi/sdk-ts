...
import {
  ...
  LendingToken,
  MarginType,
} from "@/interfaces/lending";
import { ContractAddr } from "@/dataTypes/address";
import { IConfig } from "@/interfaces";
import { Pricer } from "./pricer";

//New Logic
import { getNewTokens } from "@/UpdatedTokens";
...
export class ZkLend extends ILending implements ILending {
  ...
  async init() {
    try {
      ...
      //This will adds new tokens to zkLend so getPositions and price compute will NOT fail.
      const savedTokens = await getNewTokens();
      //Global.getTokens();

      data.forEach((pool) => {
        let collareralFactor = new Web3Number(0, 0);
        if (pool.collateral_factor) {
          collareralFactor = Web3Number.fromWei(
            pool.collateral_factor.value,
            pool.collateral_factor.decimals
          );
        }
        const savedTokenInfo = savedTokens.find(
          (t) => t.symbol == pool.token.symbol
        );
        const token: LendingToken = {
          ...
          borrowFactor: Web3Number.fromWei(
            pool.borrow_factor.value,
            pool.borrow_factor.decimals
          ),
          collareralFactor,
        };
      ...
      logger.info(
        `Initialised ${this.metadata.name} with ${this.tokens.length} tokens`
      );
   ...
   */
  async get_health_factor_tokenwise(
    lending_tokens: TokenInfo[],
    debt_tokens: TokenInfo[],
    user: ContractAddr
  ): Promise<number> {
    const positions = await this.getPositions(user);
    logger.verbose(
      `${this.metadata.name}:: Positions: ${JSON.stringify(positions)}`
    );
      ...
      .forEach((pos) => {
        ...
        if (!token) {
          throw new FatalError(
            `Token ${pos.tokenName} not found in ${this.metadata.name}`
          );
        }
        effectiveDebt = effectiveDebt.plus(
          pos.debtUSD.dividedBy(token.borrowFactor.toFixed(6)).toString()
        );
      });
      ...
      .forEach((pos) => {
        ...
        logger.verbose(
          `${this.metadata.name}:: Token: ${
            pos.tokenName
          }, Collateral factor: ${token.collareralFactor.toFixed(6)}`
        );
        effectiveCollateral = effectiveCollateral.plus(
          pos.supplyUSD
            .multipliedBy(token.collareralFactor.toFixed(6))
            .toString()
        );
      });
    logger.verbose(
      `${this.metadata.name}:: Effective collateral: ${effectiveCollateral}`
    );

    // Health factor = Effective collateral / Effective debt
    const healthFactor = effectiveCollateral
      .dividedBy(effectiveDebt.toFixed(6))
      .toNumber();
  ...
  async getPositionsSummary(user: ContractAddr): Promise<{
    ...
    const collateralUSD = pos.reduce(
      (acc, p) => acc + p.supplyUSD.toNumber(),
      0
    );
  ...
  async getPositions(user: ContractAddr): Promise<ILendingPosition[]> {
    ...
    logger.verbose(
      `${this.metadata.name}:: Positions: ${JSON.stringify(data)}`
    );
    for (let i = 0; i < data.pools.length; i++) {
      ...
      if (!token) {
        throw new FatalError(
          `Token ${pool.token_symbol} not found in ${this.metadata.name}`
        );
      }
      const debtAmount = Web3Number.fromWei(
        pool.data.debt_amount,
        token.decimals
      );
      const supplyAmount = Web3Number.fromWei(
        pool.data.supply_amount,
        token.decimals
      );
      ...
      lendingPosition.push({
        ...
        marginType: pool.data.is_collateral
          ? MarginType.SHARED
          : MarginType.NONE,
        debtAmount,
